...
document.addEventListener('keypress', (evento) => {
  const tecladoValor = evento.keyCode;
...
botones.forEach((boton) => boton.addEventListener('click', () => {
  ...
  const pantallaValor = pantalla.value;
  const ultimoValor = pantallaValor.substr(pantallaValor.length - 1);
  let resultado = 0;
  let operador;
  let num1 = 0;
  let num2 = 0;
  ...
  if (pantallaValor.indexOf(' =') > -1) {
    const operadorStr = pantallaValor.split(' ');
...
borrar.addEventListener('click', () => {
  const valor = borrarLetra(pantalla.value);
...
function borrarLetra(pantallaValor) {
  const valorBorrado = pantallaValor.slice(0, -1);
  return valorBorrado;
}
